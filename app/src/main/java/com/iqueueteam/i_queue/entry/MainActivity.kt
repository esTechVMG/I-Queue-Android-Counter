    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        sendButton.setOnClickListener {
            launch(Dispatchers.IO) {
                ...
                try {
                    var myBody:IQResponse<IQUser?,IQValidationError?>? = IQResponse(0,"",null,null)
                    ...
                    if (response.isSuccessful){
                        ...
                        CoroutineScope(Dispatchers.IO).launch {
                            runCatching {
                                val listType = object : TypeToken<IQResponse<IQUser,IQValidationError?>?>() {}.type
                                myBody = Gson().fromJson(response.errorBody()?.string(),listType)
                            }
                        }
                    }
                    Log.d("Hello","World")
